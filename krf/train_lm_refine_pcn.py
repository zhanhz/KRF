...
parser.add_argument('--gpu', type=str, default="1,2,4,5,6,7")
...
import datasets.linemod.linemod_dataset_refine_8kps as dataset_desc
        ...
        with tqdm.tqdm(range(config.n_total_epoch), desc="%s_epochs" % args.cls) as tbar, tqdm.tqdm(
            ...
            for epoch in tbar:
                ...
                for batch in train_loader:
                    ...
                    self.optimizer.zero_grad()
                    _, loss, eval_res = self.model_fn(self.model, batch, it=it)
...
def train():
    ...
    if args.checkpoint is not None:
        if args.eval_net:
            checkpoint_status = load_checkpoint(
                model, None, filename=args.checkpoint[:-8], device=device
            )
        else:
            checkpoint_status = load_checkpoint(
    ...
    trainer = Trainer(
        ...
        model_fn,
        optimizer,
        checkpoint_name=os.path.join(checkpoint_fd, "FFB6D_%s_REFINE" % args.cls),
        best_name=os.path.join(checkpoint_fd, "FFB6D_%s_REFINE_best" % args.cls),
