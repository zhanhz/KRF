...
class Dataset():
    ...
    def get_item(self, item_name):
        if "pkl" in item_name:
            ...
            pred_pose = np.load(os.path.join(self.cls_root, "add_data/{}/pred_pose/{}.npy".format(rnd_typ, name)))
        else:
            name = item_name
            ...
            with Image.open(os.path.join(self.cls_root, "add_data/pred_label/{}.png".format(item_name))) as pli:
                pred_labels = np.array(pli)
                pred_labels = (pred_labels > 0).astype("uint8")
            pred_pose = np.load(os.path.join(self.cls_root, 'add_data/pred_pose/{}.npy'.format(item_name)))
        ...
        while(True): # get input cld, key point, choose idx, gt_kp_offset, gt_pose
            ...
            mesh = self.bs_utils.get_pointxyzrgb(self.cls_id, ds_type="linemod", is_full=True)[:, :3]
            R = RT[:, :3]
            T = RT[:, 3]
    ...
    def __getitem__(self, idx):
        if self.dataset_name == 'train':
            ...
            data = self.get_item(item_name)
            if data is None: return dict(go=False)
            return data
