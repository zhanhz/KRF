...
class Basic_Utils():
    ...
    def __init__(self, config):
        ...
        self.ycb_cls_ptsxyzrgb_dict = {}
        self.ycb_cls_ptsxyzrgb_cuda_dict = {}

        self.ycb_cls_kps_dict = {}
        self.ycb_cls_ctr_dict = {}
        self.lm_cls_ptsxyz_dict = {}
        self.lm_cls_ptsxyz_cuda_dict = {}
        self.lm_cls_ptsxyzrgb_dict = {}
        self.lm_cls_ptsxyzrgb_cuda_dict = {}
    ...
    def ply_vtxclr(self, pth):
        ...
        pts = []
        for _ in range(N):
            xyz=np.float32(f.readline().split()[:3]) / 1000.0
            clr=np.float32(f.readline().split()[-4:-1]) / 255.0
            pts.append(np.concatenate([xyz,clr]))
        return np.array(pts)
    ...
    def get_pointxyzrgb(self, cls, ds_type='ycb', is_full=False):
        if ds_type == "ycb":
            cls = self.get_cls_name(cls, 'ycb')
            if cls in self.ycb_cls_ptsxyzrgb_dict.keys():
                return self.ycb_cls_ptsxyzrgb_dict[cls]
            ptxyz_ptn = os.path.join(
                self.config.ycb_root, 'models',
                '{}/pts_clr_8000.txt'.format(cls),
            )
            pointxyz = np.loadtxt(ptxyz_ptn.format(cls), dtype=np.float32)
            self.ycb_cls_ptsxyzrgb_dict[cls] = pointxyz
            return pointxyz
        else:
            ptxyz_pth = os.path.join(
                'datasets/linemod/Linemod_preprocessed/models',
            ...
            pointxyz = self.ply_vtxclr(ptxyz_pth).astype(np.float)
            dellist = [j for j in range(0, len(pointxyz))]
            if is_full:
                dellist = random.sample(dellist, max(0,len(pointxyz) - 8000))
            else:
                dellist = random.sample(dellist, len(pointxyz) - 2000)
            
            pointxyz = np.delete(pointxyz, dellist, axis=0)
            self.lm_cls_ptsxyzrgb_dict[cls] = pointxyz
            return pointxyz

    def get_pointxyzrgb_cuda(self, cls, ds_type='ycb', is_full=False):
        if ds_type == "ycb":
            if cls in self.ycb_cls_ptsxyzrgb_cuda_dict.keys():
                return self.ycb_cls_ptsxyzrgb_cuda_dict[cls].clone()
            ptsxyz = self.get_pointxyzrgb(cls)
            ptsxyz_cu = torch.from_numpy(ptsxyz.astype(np.float32)).cuda()
            self.ycb_cls_ptsxyzrgb_cuda_dict[cls] = ptsxyz_cu
            return ptsxyz_cu.clone()
        else:
            if cls in self.lm_cls_ptsxyzrgb_cuda_dict.keys():
                return self.lm_cls_ptsxyzrgb_cuda_dict[cls].clone()
            ptsxyz = self.get_pointxyzrgb(cls, ds_type, is_full)
            ptsxyz_cu = torch.from_numpy(ptsxyz.astype(np.float32)).cuda()
            self.lm_cls_ptsxyzrgb_cuda_dict[cls] = ptsxyz_cu
            return ptsxyz_cu.clone()   
