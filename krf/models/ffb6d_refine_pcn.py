...
from models.pcn import PCN_Encoder, PCN_Decoder
...
class FFB6D_REFINE(nn.Module):
    def __init__(
        ...
        self.pcn_enc = PCN_Encoder()
        self.pcn_dec = PCN_Decoder(2688)
        self.fuse = DenseFusion(2048)
    ...
    def forward(
        ...
        pcn_g = self.pcn_enc(xyz)
        repeat_g = pcn_g.unsqueeze(2).repeat(1, 1, p_emb.shape[-1]) 
        fuse_f = self.fuse(rgb_emb_c, p_emb)
        

        # ###################### prediction stages #############################
        rgbd_emb = torch.cat([fuse_f, repeat_g], dim=1) #1664 + 1024
        y_coarse, y_detail = self.pcn_dec(rgbd_emb)
        ...
        # return coarse pcld, detail pcld, pred_kp_of, pred_ctr_of
        end_points['pred_kp_ofs'] = pred_kp_ofs
        end_points['pred_ctr_ofs'] = pred_ctr_ofs
        end_points['coarse'] = y_coarse
        end_points['detail'] = y_detail
...
def main():
    ...
    n_cls = 22
    model = FFB6D_REFINE(n_cls, rndla_cfg.num_points, rndla_cfg)
    print(model)
