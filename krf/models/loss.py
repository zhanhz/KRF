...
from utils.distance.chamfer_distance import ChamferDistanceFunction
...
def of_l1_loss(
    ...
    return in_loss
def of_l1_w_loss(
        pred_ofsts, kp_targ_ofst, labels, weight,
        sigma=1.0, normalize=True, reduce=False
):
    '''
    :param pred_ofsts:      [bs, n_kpts, n_pts, c]
    :param kp_targ_ofst:    [bs, n_pts, n_kpts, c]
    :param labels:          [bs, n_pts, 1]
    :param weight:          [bs, n_kpts, n_pts, 1]
    '''
    w = (labels > 1e-8).float()
    bs, n_kpts, n_pts, c = pred_ofsts.size()
    sigma_2 = sigma ** 3
...
class OFWLoss(_Loss):
    def __init__(self):
        super(OFWLoss, self).__init__(True)

    def forward(
        self, pred_ofsts, kp_targ_ofst, labels, weight,
        normalize=True, reduce=False
    ):
        l1_loss = of_l1_w_loss(
            pred_ofsts, kp_targ_ofst, labels, weight,
            sigma=1.0, normalize=True, reduce=False
        )
