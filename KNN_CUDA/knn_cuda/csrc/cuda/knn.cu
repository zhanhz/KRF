...
/**
  ...
  *
  * @param eu_dist     calculate eu_dist or others
  ...
  */
__global__ void cuComputeDistanceGlobal(bool eu_dist, float* A, int wA,
  ...
  for (int a = begin_A, b = begin_B; a <= end_A; a += step_A, b += step_B) {
    ...
    if (eu_dist){
      ...
      if (cond2 && cond1){
        ...
        if (shared_A[6][ty] * shared_B[6][tx]){
          for (int k = 3; k < 6; ++k){
            tmp = shared_A[k][ty] - shared_B[k][tx];
            ssd1 += tmp*tmp;
          }
...
/**
  ...
  *
  * @param eu_dist       calculate euclidean distance or self-defined distance
  ...
  *
  */
void knn_device(bool eu_dist, float* ref_dev, int ref_nb, float* query_dev, int query_nb,
  ...
  cuComputeDistanceGlobal<<<g_16x16, t_16x16, 0, stream>>>(eu_dist ,ref_dev, ref_nb,
